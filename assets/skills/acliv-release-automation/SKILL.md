...
2. draft a bilingual user-facing changelog with git history only as reference
...
7. store the confirmed notes in `docs/releases/v<version>.md`
8. publish by pushing the `v<version>` tag and letting GitHub Actions build/upload all release assets
...
- Release title format:
  - `v<version>`
- Release notes:
  - manually drafted and confirmed before publish
  - stored at `docs/releases/v<version>.md`
  - copied into `release/v<version>/release-notes-v<version>.md` during local/CI builds
...
- Release build entrypoint:
  ...
  - custom notes source: `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-release.ps1 -Version <x.y.z> -ReleaseNotesPath <path>`
...
### 2. Draft and confirm the changelog before any real publish

Look up the most recent formal release tag using only tags that match `v*`.

Use git history from that tag to `HEAD` only as source material for a user-facing changelog:
...
- do not let scripts generate the `## Changes` content
...
After confirmation, save the complete release notes to:

```text
docs/releases/v<version>.md
```

The build script copies this file into `release/v<version>/release-notes-v<version>.md`. It must not synthesize the `## Changes` section from commits.
...
If local release notes are copied during rehearsal, run the repo validator:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\assert-release-notes.ps1 -Path release/v<version>/release-notes-v<version>.md
```

If the validator fails, rewrite the source notes before publish.

For CI tag releases, `build-release.ps1` requires `docs/releases/v<version>.md` to already exist and pass validation. Do not add fallback behavior that generates `## Changes` from git history.
...
- `docs/releases/v<version>.md` exists, is committed, and passes `assert-release-notes.ps1`
...
- Never generate release `## Changes` in scripts; scripts may only copy and validate confirmed notes.
...
- run release checks
- save confirmed notes to `docs/releases/v1.0.4.md`
...
- save the confirmed notes to `docs/releases/v1.0.4.md`
- do not publish yet
...
- make sure `docs/releases/v1.0.4.md` contains the confirmed bilingual notes
- run checks
